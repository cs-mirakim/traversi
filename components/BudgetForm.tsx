...
export default function BudgetForm({
  ...
  return (
    <div id="cari" className="w-full max-w-4xl mx-auto bg-white text-stone-900 rounded-3xl p-6 sm:p-8 border border-stone-200 shadow-md">
          ...
          <div className="p-4 sm:p-5 rounded-2xl bg-stone-50 border border-stone-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-2">
                <span className="text-xl sm:text-2xl font-black text-emerald-800">RM</span>
                <input
                  type="number"
                  min={500}
                  max={10000}
                  ...
                  className="w-36 sm:w-44 px-3 py-1.5 text-2xl sm:text-3xl font-black text-stone-900 bg-white border border-stone-300 rounded-xl focus:ring-2 focus:ring-emerald-600 focus:outline-none"
                />
              </div>
              <span className="text-[11px] font-medium text-stone-500 mt-1 block">
            ...
            <div className="sm:text-right border-t sm:border-t-0 pt-2 sm:pt-0 border-stone-200">
              ...
              <p className="text-xs font-bold text-emerald-800 mt-0.5">
                {budgetMode === "per_pax" 
                  ...
                  : (locale === "bm" ? `(Bersamaan ~RM${Math.round(budget / pax).toLocaleString()} seorang)` : `(~RM${Math.round(budget / pax).toLocaleString()} per pax)`)}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1">
          ...
          <div className="space-y-2 bg-stone-50 p-4 rounded-2xl border border-stone-200">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold uppercase tracking-wider text-stone-700 flex items-center gap-1.5">
                ...
                <span>{locale === "bm" ? "Tempoh (1-14 Hari)" : "Duration (1-14 Days)"}</span>
              </label>
              <span className="text-sm font-black text-emerald-800 bg-white px-2.5 py-0.5 rounded-lg border border-stone-200">
            ...
            <div className="flex items-center gap-2 pt-1">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-xl bg-white border border-stone-200 text-stone-800 font-bold hover:bg-stone-100 transition-colors"
              >
              ...
              <input
                ...
                className="flex-1 text-center py-1 text-base font-black text-stone-900 bg-white border border-stone-300 rounded-xl focus:ring-2 focus:ring-emerald-600 focus:outline-none"
              />
              <button
                type="button"
          ...
          <div className="space-y-2 bg-stone-50 p-4 rounded-2xl border border-stone-200">
            ...
            <select
              ...
              className="w-full py-2 px-3 text-xs sm:text-sm font-bold rounded-xl bg-white border border-stone-300 text-stone-900 focus:ring-2 focus:ring-emerald-600 focus:outline-none"
            >
          ...
          <div className="space-y-2 bg-stone-50 p-4 rounded-2xl border border-stone-200">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold uppercase tracking-wider text-stone-700 flex items-center gap-1.5">
                ...
                <span>{locale === "bm" ? "Bilangan Pax (1-10)" : "Travelers (1-10)"}</span>
              </label>
              <span className="text-sm font-black text-emerald-800 bg-white px-2.5 py-0.5 rounded-lg border border-stone-200">
            ...
            <p className="text-[11px] font-medium text-stone-500">
              {locale === "bm" 
                ? `${Math.ceil(pax / 2)} buah bilik hotel (kongsi 2 pax/bilik).` 
                : `${Math.ceil(pax / 2)} rooms calculated (twin sharing).`}
            </p>
          </div>
        </div>
